# Deploy V4.0.0 to PyPI and dbt Package Hub

V4.0.0 publishes three related artifacts:
...
from the same `v4.0.0` tag after the local quality gates pass.
...
python scripts/release.py verify-tag v4.0.0
...
Expected coverage is at least 80% for both codebases. V4.0.0 records 85.01% Python statement
...
poetry build
python -m zipfile --list dist/dbt_data_engineering_toolkit_compiler-4.0.0-py3-none-any.whl
```
...
/tmp/det-wheel-smoke/bin/python -m pip install \
  python/dist/dbt_data_engineering_toolkit_compiler-4.0.0-py3-none-any.whl
...
git init
git add .
git commit -m "Release dbt_data_engineering_toolkit v4.0.0"
git branch -M main
git remote add origin git@github.com:systemizing-solutions/dbt_data_engineering_toolkit.git
git push -u origin main

git tag -a v4.0.0 -m "dbt_data_engineering_toolkit v4.0.0"
...
```bash
gh release view v4.0.0 --repo systemizing-solutions/dbt_data_engineering_toolkit
...
gh pr create --repo dbt-labs/hubcap \
  ...
  --body "Adds dbt_data_engineering_toolkit v4.0.0 to dbt Package Hub."
```
