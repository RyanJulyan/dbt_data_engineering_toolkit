...
```bash
python -m pip install "dbt-data-engineering-toolkit-compiler[bigquery]==4.0.1"
python -m pip install "dbt-data-engineering-toolkit-compiler[snowflake]==4.0.1"
python -m pip install "dbt-data-engineering-toolkit-compiler[databricks]==4.0.1"
```

Use a separate virtual environment per warehouse. In particular, Spark's upstream PyHive stack
and the Databricks SQL connector currently require incompatible Thrift versions. V4.0.1 therefore
