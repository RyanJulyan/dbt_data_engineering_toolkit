...
```bash
python -m pip install "dbt-data-engineering-toolkit-compiler[bigquery]==3.0.7"
python -m pip install "dbt-data-engineering-toolkit-compiler[snowflake]==3.0.7"
python -m pip install "dbt-data-engineering-toolkit-compiler[databricks]==3.0.7"
```

Use a separate virtual environment per warehouse. In particular, Spark's upstream PyHive stack
and the Databricks SQL connector currently require incompatible Thrift versions. V3.0.7 therefore
