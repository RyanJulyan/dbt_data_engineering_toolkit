...
```bash
python -m pip install "dbt-data-engineering-toolkit-compiler[bigquery]==2.3.0"
python -m pip install "dbt-data-engineering-toolkit-compiler[snowflake]==2.3.0"
python -m pip install "dbt-data-engineering-toolkit-compiler[databricks]==2.3.0"
```

Use a separate virtual environment per warehouse. In particular, Spark's upstream PyHive stack
and the Databricks SQL connector currently require incompatible Thrift versions. V2.3.0 therefore
